...
pub(crate) fn map_stored_account(row: &sqlx::sqlite::SqliteRow) -> Result<StoredAccount, CommerceServiceError> {
    Ok(StoredAccount {
...
fn map_wallet_account(row: &sqlx::sqlite::SqliteRow) -> Result<WalletAccountItem, CommerceServiceError> {
...
pub(crate) async fn apply_points_lot_movement_for_hold(
    tx: &mut Transaction<'_, Sqlite>,
    tenant_id: i64,
    account_id: i64,
...
mod tests {
    ...
    async fn sqlite_points_debit_consumes_lots_fifo() {
        ...
        let lots = store
            .list_points_lots(
                PointsLotListQuery::new("100001", Some("0"), "1", None, None)
                    .expect("query"),
            )
            .await
            .expect("lots");
