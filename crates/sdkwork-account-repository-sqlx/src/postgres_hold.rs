...
use crate::store::{
    ...
    outbox::{
        emit_domain_outbox_postgres, OUTBOX_EVENT_TYPE_HOLD_CREATED, OUTBOX_EVENT_TYPE_HOLD_EXPIRED,
...
pub(crate) async fn reclaim_idempotency_scoped_public(
    ...
    let updated = sqlx::query(
        ...
        WHERE tenant_id = $4 AND scope = $5 AND idempotency_key = $6 AND status = 'FAILED'
        "#,
    )
    .bind(request_hash)
    .bind(now)
