...
impl SqliteCommerceAccountStore {
    ...
    pub async fn settle_account_hold(
        ...
        sqlx::query(
            r#"
            UPDATE acct_hold
            SET status = ?, settled_amount = amount, settled_at = ?, updated_at = ?, version = version + 1
            WHERE tenant_id = ? AND uuid = ? AND status = ?
            "#,
        )
    ...
    pub async fn list_account_holds(
        ...
        let rows = sqlx::query(&format!(
            ...
            WHERE hold.tenant_id = ?
              ...
              AND (? IS NULL OR hold.asset_code = ?)
    ...
    pub async fn create_account_transfer(
        ...
        if command.asset_type == CommerceAccountAssetType::Points {
            ...
            crate::sqlite_account::apply_points_lot_movement(
                &mut tx,
                crate::sqlite_account::PointsLotMovementInput {
...
async fn load_account_by_owner_asset(
    ...
    if let Some(account) = sqlx::query(
        ...
        FROM acct_account
        WHERE tenant_id = ? AND organization_id = ? AND owner_type = ? AND owner_id = ? AND asset_code = ?
        LIMIT 1
        "#,
    )
    .bind(tenant_id)
...
async fn update_account_balances(
    ...
    let update = sqlx::query(
        r#"
        UPDATE acct_account
        SET available_amount = ?, frozen_amount = ?, version = ?, updated_at = ?
...
fn map_hold_row(row: &sqlx::sqlite::SqliteRow) -> Result<AccountHoldItem, CommerceServiceError> {
    Ok(AccountHoldItem {
...
async fn load_transfer_ledger_by_journal(
    ...
    let row = sqlx::query(
        ...
        WHERE tenant_id = ? AND journal_id = ? AND account_id = ? AND direction = ?
        LIMIT 1
        "#,
    )
    .bind(tenant_id)
...
pub(crate) async fn reclaim_idempotency_scoped_public(
    ...
    let updated = sqlx::query(
        ...
        WHERE tenant_id = ? AND scope = ? AND idempotency_key = ?
          AND (status = 'FAILED' OR (status = 'LOCKED' AND locked_until <= ?))
        "#,
    )
    .bind(request_hash)
    .bind(&lock_expires)
...
async fn complete_idempotency_scoped(
    ...
    sqlx::query(
        r#"
        UPDATE acct_idempotency_record
        SET status = 'COMPLETED', target_id = ?, response_snapshot = ?, locked_until = NULL, updated_at = ?
        WHERE tenant_id = ? AND scope = ? AND idempotency_key = ?
        "#,
    )
    .bind(target_id)
...
mod tests {
    ...
    #[tokio::test]
    async fn sqlite_hold_create_and_release_restores_available() {
        let pool = migrated_pool().await;
        let store = SqliteCommerceAccountStore::new(pool);

        store
            .append_ledger_entry(
                cash_credit("credit-hold-release", "1", "100", "txn-hold-release"),
                CommerceRequestHash::new("hash-credit").expect("hash"),
            )
            .await
            .expect("credit");
        ...
        let created = store
            .create_account_hold(
                cash_hold_command("idem-hold-release", "30"),
                CommerceRequestHash::new("hash-hold").expect("hash"),
            )
            .await
            .expect("create hold");
        ...
        assert_eq!(created.account.available_amount.as_str(), "70");
        assert_eq!(created.account.frozen_amount.as_str(), "30");
        assert_eq!(created.hold.status, "held");

        let released = store
            .release_account_hold(
                ...
                .expect("command"),
                CommerceRequestHash::new("hash-release").expect("hash"),
            )
            .await
            .expect("release hold");

        assert_eq!(released.account.available_amount.as_str(), "100");
        assert_eq!(released.account.frozen_amount.as_str(), "0");
        assert_eq!(released.hold.status, "released");
    }

    #[tokio::test]
    async fn sqlite_hold_create_and_settle_reduces_frozen() {
        let pool = migrated_pool().await;
        let store = SqliteCommerceAccountStore::new(pool);

        store
            .append_ledger_entry(
                cash_credit("credit-hold-settle", "1", "100", "txn-hold-settle"),
                CommerceRequestHash::new("hash-credit-settle").expect("hash"),
            )
            .await
            .expect("credit");
        ...
        let created = store
            .create_account_hold(
                cash_hold_command("idem-hold-settle", "40"),
                CommerceRequestHash::new("hash-hold-settle").expect("hash"),
            )
            .await
            .expect("create hold");

        assert_eq!(created.account.available_amount.as_str(), "60");
        assert_eq!(created.account.frozen_amount.as_str(), "40");

        let settled = store
            .settle_account_hold(
                SettleAccountHoldCommand::new(
                    "100001",
                    &created.hold.uuid,
                    "payment",
                    "txn-settle",
                ...
                .expect("command"),
                CommerceRequestHash::new("hash-settle").expect("hash"),
            )
            .await
            .expect("settle hold");

        assert_eq!(settled.account.available_amount.as_str(), "60");
        assert_eq!(settled.account.frozen_amount.as_str(), "0");
        assert_eq!(settled.hold.status, "settled");
        assert!(settled.ledger_entry.is_some());
    }

    #[tokio::test]
    async fn sqlite_transfer_moves_balance_between_accounts() {
        let pool = migrated_pool().await;
        let store = SqliteCommerceAccountStore::new(pool);

        store
            .append_ledger_entry(
                cash_credit("credit-transfer-from", "1", "100", "txn-transfer-from"),
                CommerceRequestHash::new("hash-from").expect("hash"),
            )
            .await
            .expect("credit from");
        store
            .append_ledger_entry(
                cash_credit("credit-transfer-to", "2", "10", "txn-transfer-to"),
                CommerceRequestHash::new("hash-to").expect("hash"),
            )
            .await
            .expect("credit to");

        let from_account = store
            .list_wallet_accounts(
                WalletAccountListQuery::new(
                    "100001",
                    Some("0"),
                    "1",
                    Some(CommerceAccountAssetType::Cash),
                )
                .expect("query"),
            )
            .await
            .expect("from accounts")
            .into_iter()
            .next()
            .expect("from account");
        let to_account = store
            .list_wallet_accounts(
                WalletAccountListQuery::new(
                    "100001",
                    Some("0"),
        ...
        let outcome = store
            .create_account_transfer(
                CreateAccountTransferCommand::new(
                    "100001",
                    Some("0"),
                    &from_account.id,
                    &to_account.id,
                ...
                .expect("command"),
                CommerceRequestHash::new("hash-transfer").expect("hash"),
            )
            .await
            .expect("transfer");

        assert_eq!(outcome.from_account.available_amount.as_str(), "75");
        assert_eq!(outcome.to_account.available_amount.as_str(), "35");
